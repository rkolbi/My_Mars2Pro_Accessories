...
·  [PRINTER MAINTENANCE & TROUBLESHOOTING](#nut_and_boltMAINTENANCE-AND-TROUBLESHOOTING)
...
:bulb:To allow the tug of war to work in favor of yielding a successful print, the FEP has to remain non-stick and flexible, the resin has to stay viscous, and the build plate needs to bond to the resin. Assuming the item has been orientated and sliced correctly, the easiest way to ensure this is to keep the build plate and FEP clean and operate the printer at a room temperature of 21'C and above. Regarding temperature, always check the resin manufacturer's data as some require it to be a bit higher.
